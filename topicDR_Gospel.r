...
#=========================================================
#Dendo for Dissimliar books 9 x 9
#=========================================================
#Get just the books in clsuter containing disisimilar books 
#from the initial dendrogram
DS_similarity<-sorted_similarity[c(1,2,3,4,5,7,19,23,27),c(1,2,3,4,5,7,19,23,27)]

# Convert similarity to distance (1 - cosine similarity)
gs_dist <- as.dist(1 - DS_similarity)

# Perform hierarchical clustering
hc <- hclust(gs_dist, method = "ward.D2")

# Plot the dendrogram
plot(hc, main = "Clustering the disimilar", xlab = "Books")

#=======================================================
#Analyze the DS cosine matrix
#=======================================================

#first statistical counts of how many have greater 
#similairty than each Synoptic
MT_LK_sim <- DS_similarity[1,3]
MT_LK_count <- sum(DS_similarity > MT_LK_sim)
#61 of the 81 have topic distributions that are more similar 
#we would expect with the agreement of shared content
#MT_LK would be more simkilar if lieterary dependence 
#were true not less

MT_MR_sim <- DS_similarity[1,2]
MT_MR_count <- sum(DS_similarity > MT_MR_sim)
#71 of the 81 have topic distributions that are more similar 
#we would expect with the agreement of shared content
#MT_MR would be more simkilar if lieterary dependence 
#were true not less

LK_MR_sim<- DS_similarity[3,2]
LK_MR_count <- sum(DS_similarity > LK_MR_sim)
#57 of the 81 have topic distributions that are more similar 
#we would expect with the agreement of shared content
#MT_MR would be more simkilar if lieterary dependence 
#were true not less
